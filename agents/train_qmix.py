...
from agents.signal_agent import AGENTS                               # agent names (module also frozen)
from agents.qmix_agent import QMIXTrainer                           # value-based CTDE robustness arm (agent.learner=qmix)
...
def run_training(cfg: DictConfig):
    """QMIX training with the FULL SIGNAL experiment harness inlined: env builders, refs, held-out
    gate, best-checkpoint selection, patience, budget milestones, checkpoint payload. This file was
    produced by MECHANICAL MERGE from agents/train_common.py (the MAPPO harness) with only the
    MAPPO branch removed, and certified behaviorally identical by a same-seed tensor-level
    checkpoint comparison against the common-harness path at freeze time. Any post-freeze edit
    here must be mirrored in train_common.py and re-certified -- the sign-concordance claim
    depends on identical selection rules across learners."""
    learner = "qmix"
    ...
    # NOTE deliberately NOT stamped into A/checkpoint: mutating the config dict changes checkpoint
    # bytes and breaks the byte-identity certificate for the frozen MAPPO instrument. Dump/eval
    # tooling routes QMIX checkpoints by ARM NAME (the sweep's Phase-G list), not by payload content.
    if signal_csvlog.csvlog_enabled():
        raise SystemExit("SIGNAL_CSVLOG is not supported for QMIX "
                         "(the scalar logger reads MAPPO-specific internals). Unset it for QMIX runs.")
    trainer = QMIXTrainer(A, n_agents=n, obs_dim=obs_dim, state_dim=state_dim, adj=adj, device=device)
