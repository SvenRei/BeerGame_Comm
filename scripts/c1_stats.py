...
DATA MODEL
  signal : {seed(int): {lambda(float): mean_cost(float)}}   # one per-lambda vector per seed,
...
Run:
  ...
  python scripts/c1_stats.py report --signal-dir results/signal_c1 \
...
# The C1 report
# ==============================================================================
def _common_lambdas(signal, rungs, keys, lambdas):
    sets = [set(signal[s]) for s in signal] + [set(rungs[k]) for k in keys if k in rungs]
...
def summarize(signal, rungs, bar_key="BAR_static", ceiling_key="Oracle", bayes_key="Bayes",
    ...
    signal = {int(s): {float(k): float(v) for k, v in d.items()} for s, d in signal.items()}
    seeds = sorted(signal)
    lams = _common_lambdas(signal, rungs, [bar_key, ceiling_key], lambdas)
    ...
    D = np.array([float(np.mean([signal[s][l] for l in lams])) for s in seeds])
    gap = (bar - D) / denom
    ...
    for l in lams:
        ...
        if has_bayes:
            rec["bayes"] = bay_l[l]
            rec["signal_minus_bayes"] = float(d_l.mean() - bay_l[l])
        per_lambda[l] = rec

    gap_ci = bootstrap_ci(gap, n_boot=n_boot, ci=ci)
    iqm_ci = bootstrap_ci(gap, stat=iqm, n_boot=n_boot, ci=ci)
    agg = {
        "signal_cost_mean": float(D.mean()),
        "signal_cost_ci": bootstrap_ci(D, n_boot=n_boot, ci=ci),
    ...
    if has_bayes:
        # paired one-sample test over seeds: is SIGNAL below the (fixed) Bayes-adaptive cost?
        # diff = Bayes - SIGNAL_cost(seed)  ->  positive = SIGNAL cheaper.
        ...
        vs["beats_bayes_ci"] = bool(vs_ci[0] > 0.0)       # CI of (Bayes - SIGNAL) excludes 0
        agg["vs_bayes"] = vs
...
def print_report(rep):
    ...
    print(f"  SIGNAL mean cost     : {a['signal_cost_mean']:.1f}  95% CI {_ci1(a['signal_cost_ci'])}")
    ...
    le1 = "OK (oracle bounds SIGNAL)" if a["gap_le1_check"] else \
          "VIOLATED -> SIGNAL beats the STATIC oracle somewhere (finite-horizon effect; T3.3)"
    print(f"  Gap <= 1 check      : {le1}")

    if "vs_bayes" in a:
        vb = a["vs_bayes"]
        verdict = "SIGNAL significantly below Bayes" if vb["beats_bayes_ci"] else \
                  "not significant (CI includes 0)"
        print("-" * 78)
        print(f"  [SECONDARY] SIGNAL vs the Bayes naive-adaptation FLOOR (paired over seeds, +=SIGNAL cheaper).")
        ...
        print(f"    mean(Bayes - SIGNAL) = {vb['mean_diff']:+.1f}   95% CI {_ci1(vb['diff_ci'])}  -> {verdict}")
    ...
    print("-" * 78)
    print(f"  per-lambda:")
    hdr = f"    {'lambda':>7}{'SIGNAL':>10}{'95% CI':>17}{'BAR':>8}"
    hdr += f"{'Bayes':>8}" if rep["has_bayes"] else ""
    hdr += f"{'BestStat':>8}{'Gap':>8}{'Gap CI':>16}{'<=1':>5}"
    print(hdr)
    for l in rep["lambdas"]:
        r = rep["per_lambda"][l]
        row = f"    {l:>7g}{r['signal_mean']:>10.1f}{_ci1(r['signal_ci']):>17}{r['bar']:>8.0f}"
        if rep["has_bayes"]:
            row += f"{r['bayes']:>8.0f}"
...
def _report_cli(argv):
    ...
    ap.add_argument("--signal-dir", default="results/signal_c1",
                    help="dir of seed{S}.json (per-seed {lambda: cost}); see eval_signal.py --dump-c1")
    ap.add_argument("--refs", default="results/baselines_regime_v2.json")
    ap.add_argument("--lambdas", nargs="+", type=float, default=None)
    args = ap.parse_args(argv)
    signal = load_signal_dir(args.signal_dir)
    if not signal:
        print(f"no seed*.json found in {args.signal_dir}. Generate them with eval_signal.py --dump-c1.")
        return
    rungs = load_rungs(args.refs)
    print_report(summarize(signal, rungs, lambdas=args.lambdas))
...
def _selftest():
    ...
    def signal_at(cost_fn):
        return {s: {l: float(cost_fn(s, l)) for l in lams} for s in range(5)}

    # Case A: SIGNAL == Oracle -> Gap == 1, oracle bound valid.
    rep = summarize(signal_at(lambda s, l: 2000.0), rungs, n_boot=2000)
    assert abs(rep["aggregate"]["gap_mean"] - 1.0) < 1e-6, rep["aggregate"]["gap_mean"]
    assert rep["aggregate"]["gap_le1_check"] is True
    print(f"  A) SIGNAL=Oracle      -> gap={rep['aggregate']['gap_mean']:.3f} (==1), Gap<=1 OK  PASS")

    # Case B: SIGNAL == BAR -> Gap == 0, CI must NOT exclude 0.
    rep = summarize(signal_at(lambda s, l: 4000.0), rungs, n_boot=2000)
    assert abs(rep["aggregate"]["gap_mean"]) < 1e-6
    assert rep["aggregate"]["gap_ci_excludes_0"] is False
    print(f"  B) SIGNAL=BAR         -> gap={rep['aggregate']['gap_mean']:.3f} (==0), CI!>0  PASS")

    # Case C: SIGNAL between, with per-seed noise -> 0<gap<1, CI brackets the mean, beats Bayes.
    rng = np.random.default_rng(1)
    noise = {s: rng.normal(0, 60) for s in range(5)}
    rep = summarize(signal_at(lambda s, l: 3000.0 + noise[s]), rungs, n_boot=4000)
    g = rep["aggregate"]["gap_mean"]; lo, hi = rep["aggregate"]["gap_ci"]
    assert 0.0 < g < 1.0 and lo <= g <= hi
    assert rep["has_bayes"] and rep["aggregate"]["vs_bayes"]["mean_diff"] < 0  # Bayes(2500)-SIGNAL(3000)<0
    print(f"  C) SIGNAL=3000(+noise)-> gap={g:.3f} in (0,1), CI=[{lo:.3f},{hi:.3f}], "
          f"vs-Bayes mean_diff={rep['aggregate']['vs_bayes']['mean_diff']:+.0f} (Bayes cheaper)  PASS")

    # Case D: SIGNAL below the static oracle -> Gap>1, validity flag must trip; SIGNAL beats Bayes.
    rep = summarize(signal_at(lambda s, l: 1500.0 + 20 * (s - 2)), rungs, n_boot=2000)
    assert rep["aggregate"]["gap_mean"] > 1.0
    assert rep["aggregate"]["gap_le1_check"] is False
    vb = rep["aggregate"]["vs_bayes"]
    assert vb["mean_diff"] > 0 and vb["diff_ci"][0] > 0  # Bayes(2500)-SIGNAL(~1500)>0, CI excludes 0
    print(f"  D) SIGNAL=1500        -> gap={rep['aggregate']['gap_mean']:.3f} (>1), Gap<=1 VIOLATED "
