...
Run:
  python scripts/run_confirmatory_report.py --signal-dir results/signal_c1 --refs results/baselines_regime_v2.json
...
from scripts.c1_stats import (load_rungs, mean_refs, load_signal_dir, summarize, print_report,
...
# ----------------------------------------------------------------------- loaders
def load_bw_dir(signal_dir):
    """seed{S}_bw.json -> {seed: {lambda: {echelon: BW_cum}}}."""
    out = {}
    for p in sorted(glob.glob(os.path.join(signal_dir, "seed*_bw.json"))):
...
def signal_bullwhip(bw_by_seed, lambdas):
...
# ----------------------------------------------------------------------- prereg check
def prereg_check(signal, lambdas, min_seeds=MIN_SEEDS):
    msgs = []
    n = len(signal)
    if n < min_seeds:
        msgs.append(f"WARNING: {n} seeds < preregistered minimum {min_seeds} (pilot only; CIs wide).")
    have = sorted({float(l) for d in signal.values() for l in d})
...
# ----------------------------------------------------------------------- report
def run(signal_dir, refs_json, lambdas=HELDOUT_LAMBDAS, bullwhip_episodes=100, comm_dirs=None,
        out_path="results/confirmatory_report.json", min_seeds=MIN_SEEDS, verbose=True):
    signal = load_signal_dir(signal_dir)
    bw_by_seed = load_bw_dir(signal_dir)
    rungs = load_rungs(refs_json)
    raw = json.load(open(refs_json))
    bar_levels = raw.get("meta", {}).get("bar_levels")

    warnings = prereg_check(signal, lambdas, min_seeds)
    rep = summarize(signal, rungs, lambdas=lambdas)
    if verbose:
        for w in warnings:
            print(f"  [prereg] {w}")
        print_report(rep)

    # ---- bullwhip table (SIGNAL from dumps; static-BAR + Bayes computed offline) ----
    bull = {}
    if bw_by_seed:
        bull["SIGNAL"] = signal_bullwhip(bw_by_seed, lambdas)
    ...
    if verbose:
        ...
        print("    (static-BAR ~1 = bullwhip-free; Bayes floor amplifies; SIGNAL should stay low while "
              "matching the regime.)")
...
def _selftest():
    ...
    ddir = os.path.join(tmp, "signal_c1"); os.makedirs(ddir, exist_ok=True)
...
def main():
    if len(sys.argv) > 1:
        ...
        a = ap.parse_args()
        run(a.signal_dir, a.refs, lambdas=a.lambdas, bullwhip_episodes=a.bullwhip_episodes,
